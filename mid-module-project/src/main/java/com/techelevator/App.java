...
public class App {
    ...
    private void loadData() {
        ...
        for (int i = 0; i < dataset.length; i++) {
            ...
            String[] splitStringArray = dataset[i].split(FIELD_DELIMITER, 4);

            titles.add(splitStringArray[0]);
            authors.add(splitStringArray[1]);
            publishedYears.add(Integer.parseInt(splitStringArray[2]));
            prices.add(new BigDecimal(splitStringArray[3]));
            ...
            if (mainMenuSelection == 1) {
                ...
                while (true) {
                    ...
                    if (searchBooksMenuSelection == 1) {
                         ...
                         */
                        // displayTitlesList(titles);
                        List<Integer> searchList = filterByTitle(filterTitle);
                        ...
                        List<Integer> searchFoundAuthor = filterByAuthor(filterAuthor);
                        displaySearchResults(searchFoundAuthor);
                        ...
                        List<Integer> searchPublishedYears = filterByPublishedYear(filterYear);
                        displaySearchResults(searchPublishedYears);
                        ...
                        List<Integer> searchYearRanges = filterByPublishedYearRange(filterFromYear, filterToYear);
                        displaySearchResults(searchYearRanges);
                         ...
                         */
                        // displayPublishedYearsList(publishedYears);
                        List<Integer> mostRecentBooks = findMostRecentBooks();
                        displaySearchResults(mostRecentBooks);
                        ...
                        List<Integer> pricesListed = filterByPrice(filterPrice);
                        displaySearchResults(pricesListed);
    ...
    private void displaySearchResults(List<Integer> indexes) {
        for (int index : indexes) {
            System.out.println(titles.get(index) + ": " + authors.get(index) + ": "
                    + publishedYears.get(index) + ": $" + prices.get(index));
    ...
    private List<Integer> filterByTitle(String filterTitle) {
        ...
        for (int i = 0; i < titles.size(); i++) {
            ...
            String findingTitleMatch = titles.get(i).toLowerCase();

            if (findingTitleMatch.contains(filterTitle.toLowerCase())) {
                listResults.add(i);
            }
        }
        return listResults;
    ...
    private List<Integer> filterByAuthor(String filterAuthor) {
        ...
        List<Integer> findingAuthor = new ArrayList<>();

        for (int i = 0; i < authors.size(); i++) {

            String findingAuthorMatch = authors.get(i).toLowerCase();

            if (findingAuthorMatch.contains(filterAuthor.toLowerCase())) {
                findingAuthor.add(i);
            }
        }
        return findingAuthor;
    }
    ...
    private List<Integer> filterByPublishedYear(int filterYear) {
        ...
        List<Integer> yearFoundList = new ArrayList<>();

        for (int i = 0; i < publishedYears.size(); i++) {

            int findingYearMatch = publishedYears.get(i);

            if (findingYearMatch == filterYear) {
                yearFoundList.add(i);
            }
        }

        return yearFoundList;
    }
    ...
    private List<Integer> filterByPublishedYearRange(int filterFromYear, int filterToYear) {
        ...
        List<Integer> yearFoundRangeList = new ArrayList<>();

        for (int i = 0; i < publishedYears.size(); i++) {

            int findingYearRange = publishedYears.get(i);

            if (findingYearRange >= filterFromYear && findingYearRange <= filterToYear) {
                yearFoundRangeList.add(i);
            }
        }
        return yearFoundRangeList;
    }
    ...
    private List<Integer> findMostRecentBooks() {
        ...
        List<Integer> mostRecentBooksList = new ArrayList<>();

        int largestYear = Collections.max(publishedYears);

        for(int i = 0; i < publishedYears.size(); i++) {
            if(publishedYears.get(i) == largestYear) {
                mostRecentBooksList.add(i);
            }
        }

        return mostRecentBooksList;
    ...
    private List<Integer> filterByPrice(double filterPrice) {
        ...
        for(int i = 0; i < prices.size(); i++) {
            ...
            BigDecimal findingPriceMatch = prices.get(i);
            BigDecimal convertedFilterPrice = BigDecimal.valueOf(filterPrice);

            if(findingPriceMatch.compareTo(convertedFilterPrice) <= 0) {
                pricesList.add(i);
            }
    ...
    private List<Integer> filterByPriceRange(double filterFromPrice, double filterToPrice) {
        ...
        List<Integer> priceRangeListFound = new ArrayList<>();

        for(int i = 0; i < prices.size(); i++) {

            Double convertedBigDecimalToDouble = prices.get(i).doubleValue();

            if(convertedBigDecimalToDouble >= filterFromPrice && convertedBigDecimalToDouble <= filterToPrice) {
                priceRangeListFound.add(i);
            }
        }

        return priceRangeListFound;
    }
    ...
    private List<Integer> findLeastExpensiveBooks() {
        ...
        BigDecimal leastExpensivePrice = Collections.min(prices);

        for(int i = 0; i < prices.size(); i++) {
            if(prices.get(i).equals(leastExpensivePrice)) {
                lowestPrices.add(i);
            }
        }
